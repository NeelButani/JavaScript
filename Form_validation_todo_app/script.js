...
const validate = (event) => {
  ...
  if (isFormValid() === true) {
    setValuesToLocalStorage(userNameVal, emailVal, phoneNumberVal, passwordVal);
    form.submit();
  } else {
...
const userNameValidation = (userNameVal) => {
  ...
  // length validation
    ...
    setErrorMsg(
      userName,
      " &bull; Username should be between 3 to 10 characters."
    );
  }
  ...
  // Should not start with number or underscore
    ...
    setErrorMsg(
      userName,
      "&bull; Username should not start with number and underscore"
    );
  } else {
